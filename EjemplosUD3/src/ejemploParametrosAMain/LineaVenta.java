package ejemploParametrosAMain;

public class LineaVenta {

	private Producto p;
	private int cantidad;
	private double subTotal;

	public LineaVenta(Producto p, int cantidad, double subTotal) {
		super();
		this.p = p;
		this.cantidad = cantidad;
		this.subTotal = subTotal;
	}

	public LineaVenta(Producto p, int cantidad) {
		super();
		this.p = p;
		this.cantidad = cantidad;
	}

	public double calcularSubTotal(double por) {
		double resultado = 0.0;
		/*
		 * muy mal seria resultado=cantidad*p.getPrecioBase()*precio y volver a escribir
		 * el código practicamente
		 */
		resultado = cantidad * p.calcularPVP(por);
		subTotal = resultado;
		return resultado;

	}

	public Producto getP() {
		return p;
	}

	public void setP(Producto p) {
		this.p = p;
	}

	public int getCantidad() {
		return cantidad;
	}

	public void setCantidad(int cantidad) {
		this.cantidad = cantidad;
	}

	public double getSubTotal() {
		return subTotal;
	}

	public void setSubTotal(double subTotal) {
		this.subTotal = subTotal;
	}

	public String toString() {
		return "LineaVenta [p=" + p + ", cantidad=" + cantidad + ", subTotal=" + subTotal + "]";
	}

}